...
export const getAiPoweredMatches = async (currentUser, candidates) => {
    ...
    // Performance: Add caching to reduce Gemini API calls
    const cacheKey = `matches:${currentUser._id}:${currentUser.language || 'any'}`;
    const cachedMatches = await getCache(cacheKey);
    if (cachedMatches) {
        // Filter cached results to only include candidates still in the potential list
        const candidateIds = new Set(candidates.map(c => c._id.toString()));
        const validCachedMatches = cachedMatches.filter(m => candidateIds.has(m.id));
        if (validCachedMatches.length > 0) {
            return validCachedMatches;
        }
    }
...
/**
 * Generates an initial, empathetic response for a newly filed dispute.
 * @param {object} disputeDetails - Details about the dispute.
 * @param {string} disputeDetails.reason - The reason for the dispute.
 * @param {string} disputeDetails.description - The user's description of the issue.
 * @returns {Promise<string>} - A promise that resolves to the AI-generated text response.
 */
export const getInitialDisputeResponse = async (disputeDetails) => {
    const prompt = `
    You are "Gemini Support," a helpful and empathetic AI assistant for a knowledge exchange platform called Knowle.
    A user has just filed a dispute and needs immediate acknowledgment.
    The user's reported issue is:
    - Reason: "${disputeDetails.reason}"
    - Description: "${disputeDetails.description}"

    Your task is to generate a supportive initial response. Your response must:
    1. Acknowledge the user's report and the reason they provided.
    2. Express empathy for their frustrating experience.
    3. Provide a clear, immediate next step. Specifically, ask them to upload any relevant evidence like screenshots or conversation logs to help the human support team investigate.
    4. Reassure them that a ticket has been created and a human team member will review their case.
    5. Maintain a calm, professional, and helpful tone.
    
    Return only the plain text of your response.
    `;
    ...
    try {
        ...
        console.error("Error calling Gemini API for dispute response:", error);
        // Fallback message in case the AI fails
        return "Thank you for submitting your report. We have received it and a member of our support team will review it shortly. To help us resolve this faster, please upload any relevant screenshots or evidence to this chat.";
    }
};