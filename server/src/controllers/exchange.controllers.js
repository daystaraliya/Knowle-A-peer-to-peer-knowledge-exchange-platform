...
const confirmCompletion = asyncHandler(async (req, res) => {
    ...
    if (exchange.status !== 'accepted') {
        throw new ApiError(400, "Only an accepted exchange can be marked as completed.");
    }
    ...
    if (exchange.initiatorCompleted && exchange.receiverCompleted) {
        ...
        if (initiatorSkillNode) {
            await User.findByIdAndUpdate(exchange.initiator, { $addToSet: { completedSkillNodes: initiatorSkillNode._id } });
        }
        ...
        if (receiverSkillNode) {
            await User.findByIdAndUpdate(exchange.receiver, { $addToSet: { completedSkillNodes: receiverSkillNode._id } });
        }
    ...
    return res.status(200).json(new ApiResponse(200, updatedExchange, "Completion confirmed successfully."));
});
// MODIFICATION END
