...
import { sanitizeHTML, isValidURL } from "../utils/validation.js";
...
const createResource = asyncHandler(async (req, res) => {
    ...
    if (type === 'link') {
        ...
        if (!isValidURL(url)) {
            throw new ApiError(400, "Please provide a valid and secure URL (http/https).");
        }
    }
...
const updateResource = asyncHandler(async (req, res) => {
    ...
    if (resource.type === 'link' && url) {
        if (!isValidURL(url)) throw new ApiError(400, "Please provide a valid and secure URL.");
        resource.url = url;
    }
