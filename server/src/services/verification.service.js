...
export const checkAndAwardVerifiedSkill = async (userId, topicId, io) => {
    try {
        ...
        const existingVerification = await VerifiedSkill.findOne({
            user: userId,
            topic: topicId,
        });
        ...
        console.log(
            `✅ Awarded verified skill for topic ${topicId} to user ${userId}`
        );
        ...
        console.error(
            `Error in skill verification service for user ${userId} and topic ${topicId}:`,
            error
        );
    }
};
