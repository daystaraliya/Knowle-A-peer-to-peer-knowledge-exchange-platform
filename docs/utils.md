...
The `server/src/utils/` directory contains a set of helper functions and classes that provide reusable logic across the application. These utilities help keep the controller logic clean and enforce consistent patterns for error handling, API responses, external service interactions, and caching.
...
-   **Purpose:** A module to abstract interactions with the Redis caching server.
-   **Implementation:** It initializes a Redis client using the connection URL from environment variables. It exports helper functions: `initRedis` (to be called on server start), `getCache` (to retrieve and parse data), `setCache` (to stringify and store data with an expiration), and `delCache` (to invalidate a cache entry).
-   **Usage:** Used in controllers to reduce database load.

    ```javascript
    // In user.controllers.js
    import { getCache, setCache, delCache } from "../utils/cache.js";

    // At the start of a GET controller
    const cachedUser = await getCache(`user:${userId}`);
    if (cachedUser) return res.status(200).json(new ApiResponse(200, cachedUser));
    
    // After a successful database query
    await setCache(`user:${userId}`, userFromDb, 1800); // Cache for 30 mins
    
    // After an update (PATCH/POST/DELETE)
...
-   **Implementation:** Contains functions like `isValidEmail` and `isValidPassword` that use regular expressions to check if strings match required formats. It also includes a basic `sanitizeHTML` function to prevent XSS attacks on user-submitted text.
