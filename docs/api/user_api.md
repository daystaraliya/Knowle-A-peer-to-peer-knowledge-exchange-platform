...
-   **Success Response (200):**
    ...
    ```json
    {
      ...
      "data": {
        ...
        "achievements": [{ "_id": "...", "name": "First Exchange", "..."}],
        "proficiencies": [{ "topic": { "_id": "...", "name": "React" }, "proficiency": "Advanced" }]
      },
...
### GET /api/v1/users/public/slug/:slug

Fetches a user's public profile by their unique slug.

-   **Method:** `GET`
-   **URL:** `/api/v1/users/public/slug/:slug`
-   **Authentication:** None.
-   **Request Body:** None
-   **Success Response (200):** The data returned depends on the user's privacy settings.
...
Updates the profile details (fullName, bio, languages) of the currently authenticated user.
...
### POST /api/v1/users/reviews/regenerate

Triggers a background job to regenerate the user's AI-powered review summary.

-   **Method:** `POST`
-   **URL:** `/api/v1/users/reviews/regenerate`
-   **Authentication:** Requires valid JWT token in cookie.
-   **Request Body:** None.
-   **Success Response (202):** Acknowledges that the process has started. The user will be notified via WebSocket when it's complete.

    ```json
    {
        "statusCode": 202,
        "data": null,
        "message": "AI review analysis has been initiated. You will be notified upon completion.",
        "success": true
    }
    ```
...
Saves or updates a user's web push notification subscription.

-   **Method:** `POST`
-   **URL:** `/api/v1/users/push/subscribe`
-   **Authentication:** Requires valid JWT token in cookie.
-   **Request Body:** The `PushSubscription` object obtained from the browser's Push API.
